...
export default {
  ...
  setup(props){
    ...
    const selected         = ref(false); // применяет класс для выбранного профиля
    ...
    return {grid, showFilter, activeProfile, profileName, creatingProfile, selected, filtered, dontCloseFilter, showValue, onSubmit, clear, showFilterDropMenu, createProfile, deleteProfile, hideFilter, saveUpdatedProfile, showFilterDropMenuButton}
  },
              ...
              h(
                  ...
                  [
                    h(
                        'button',
                        {
                          class : ['vdg_profileRow_element',{selected : this.selected},'el-button','el-button--default',{selected : i == this.activeProfile}],
                          onClick : $event => {
            ...
            h(
                ...
                h(
                    ...
                    [
                      ...
                      //     }
                      // )
                       this.showValue.length > 0 ? '' : h('span',{class : 'vdg_inputFilter'}, 'Фильтр + поиск')
                    ]
                )
            ),
            h( Transition,
